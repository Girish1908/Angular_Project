...
export class ProfileComponent implements OnInit {
  ...
  updateRegistration(registration: any) {
    ...
    this.http.patch(`http://localhost:8080/user/registration/${registration.name}`, registration).subscribe(
      response => {
  ...
  deleteRegistration(name: string) {
    if(confirm('confirm to delete')){
    this.http.delete(`http://localhost:8080/user/registration/${name}`).subscribe(
      response => {
